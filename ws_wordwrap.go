...
//export ws_wordwrap_init
func ws_wordwrap_init(initid *C.UDF_INIT, args *C.UDF_ARGS, message *C.char) C.my_bool {
	...
	if typeslice[2] != C.STRING_RESULT {
		return 1
	}
...
//export ws_wordwrap
func ws_wordwrap(initid *C.UDF_INIT, args *C.UDF_ARGS, result *C.char, length *C.ulong, isNull *C.char, error *C.char) *C.char {
	...
	// -----
	// ここが処理本体
	limit, _ := strconv.Atoi(argsString[1])
	rslt := _func.Wordwrap(argsString[0], limit, argsString[2])
	// -----
...
//export ws_wordwrap_deinit
func ws_wordwrap_deinit(initid *C.UDF_INIT) {
	C.free(unsafe.Pointer(initid.ptr))
}
